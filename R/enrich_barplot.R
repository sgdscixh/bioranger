...
#' @param use_odds_ratio Logical, whether to use \code{OddsRatio } for the x-axis. If \code{FALSE}, \code{Count} will be used. Default is \code{FALSE}.
#'
...
#'
#' @details
#' The function uses \code{ggplot2} to create a faceted bar plot of enrichment results. By default, it plots the \code{Count} of genes in each enriched term on the x-axis. Setting \code{use_odds_ratio = TRUE} will plot the \code{OddsRatio } instead. The bars are ordered within each facet based on the \code{p.adjust} values.
#'
...
enrichment_barplot <- function(
  ...
  p <- ggplot(data, aes(
    ...
    labs(
      x = if (use_odds_ratio) "Gene Ratio" else "Count",
      y = NULL,
